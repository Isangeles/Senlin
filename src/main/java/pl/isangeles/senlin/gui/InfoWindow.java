...
public class InfoWindow extends InterfaceObject {
   ...
   */
  public InfoWindow(GameContainer gc, String textInfo)
  ...
  private float getCorrectX(float x) {
    if (x + getScaledWidth() >= Global.getCameraSize().width) x -= text.getTextWidth();
  ...
  private float getCorrectY(float y) {
    if (y + getScaledHeight() >= Global.getCameraSize().height) y -= text.getTextHeight();

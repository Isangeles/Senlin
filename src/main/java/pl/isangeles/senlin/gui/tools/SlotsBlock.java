...
public class SlotsBlock {
  ...
  public void draw(float x, float y) {
    for (int i = 0; i < slots.length; i++) {
      for (int j = 0; j < slots[i].length; j++) {
        slots[i][j].draw(
            x + (j * (slots[i][j].getScaledWidth() + Coords.getDis(5))),
            y + (i * (slots[i][j].getScaledWidth() + Coords.getDis(8))),
            false);
      }
    }
  }
  ...
  public boolean insertContent(SlotContent content) {
    for (int i = 0; i < slots.length; i++) {
      for (int j = 0; j < slots[i].length; j++) {
        if (slots[i][j].insertContent(content)) return true;
        else continue;
      }
    }
    return false;
  ...
  public boolean insertContentInto(SlotContent content, int row, int column) {
    if (slots[row][column].isEmpty()) return slots[row][column].insertContent(content);
    else return false;
  }
  /**
  ...
  public boolean removeContent(SlotContent content) {
    for (Slot slot : allSlots) {
      if (slot.getContent().contains(content)) return slot.removeContent(content);
    }
    return false;
  }
  ...
  public Slot getDragged() {
    for (Slot slotsLine[] : slots) {
      for (Slot ss : slotsLine) {
        if (ss.isContentDragged()) return ss;
      }
    }
    return null;
  }
  ...
  public int[] getSlotPos(Slot slotToCheck) {
    ...
    for (Slot[] line : slots) {
      for (Slot slot : line) {
        if (slot == slotToCheck) return new int[] {row, column};
        column++;
      }
      row++;
      column = 0;

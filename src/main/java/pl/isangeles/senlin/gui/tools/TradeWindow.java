...
class TradeWindow extends InterfaceObject implements UiElement, MouseListener {
   ...
   */
  public TradeWindow(GameContainer gc, Character player)
    ...
    if (openReq) {
      ...
      if (button == Input.MOUSE_LEFT_BUTTON) {
        ...
        if (slotsBuy.getDragged() != null) {
          ...
          if (moSlot != null) moveItem(dSlot, moSlot);
        }
  ...
  private boolean trade() {
    ...
    if (((trader.getInventory().getCashValue() + buyValue) - sellValue) >= 0
        && ((buyer.getInventory().getCashValue() + sellValue) - buyValue) >= 0) {
      if (tradeValue <= 0) {
        deal();
        return true;
      } else return false;

...
class CharacterWindow extends InterfaceObject implements UiElement, MouseListener {
  ...
  public void open() {
    ...
    for (int i = 0; i < info.length && i < 9; i++) {
      if (i < 5) upInfoA.addText(info[i]);
      else upInfoB.addText(info[i]);
    }
    for (int i = 8; i < info.length; i++) {
      if (i < 13) downInfoA.addText(info[i]);
      else downInfoB.addText(info[i]);
    }
  }
  /* (non-Javadoc)
  ...
  private String[] getCharStats() {
    return new String[] {
      player.getName(),
      TConnector.getText("ui", "levelName") + ": " + player.getLevel(),
      TConnector.getText("ui", "expName")
          + ": "
          + player.getExperience()
          + "/"
          + player.getMaxExperience(),
      TConnector.getText("ui", "guildName") + ": " + player.getGuild().toString(),
      TConnector.getText("ui", "hpName") + ": " + player.getHealth(),
      TConnector.getText("ui", "manaName") + ": " + player.getMagicka(),
      TConnector.getText("ui", "lpName") + ": " + player.getLearnPoints(),
      TConnector.getText("ui", "hastName") + ": " + player.getAttributes().getHaste(),
      TConnector.getText("ui", "concName") + ":" + player.getAttributes().getConcentration(),
      TConnector.getText("ui", "spName") + ": " + player.getAttributes().getSpellPower(),
      TConnector.getText("ui", "dmgName")
          + ": "
          + player.getDamage()[0]
          + "-"
          + player.getDamage()[1],
      TConnector.getText("ui", "armRat") + ": " + player.getArmorRating(),
      TConnector.getText("ui", "dodgeCh") + ": " + player.getDodgeChance() + "%",
      TConnector.getText("ui", "attStrName") + ": " + player.getAttributes().getStr(),
      TConnector.getText("ui", "attConName") + ": " + player.getAttributes().getCon(),
      TConnector.getText("ui", "attDexName") + ": " + player.getAttributes().getDex(),
      TConnector.getText("ui", "attIntName") + ": " + player.getAttributes().getInt(),

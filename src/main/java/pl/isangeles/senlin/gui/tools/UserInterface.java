...
public class UserInterface implements MouseListener, KeyListener, SaveElement {
   ...
   */
  public UserInterface(GameContainer gc, CommandInterface cli, Character player, GameWorld gw)
    ...
    bBar =
        new BottomBar(gc, gw, igMenu, charWin, inventory, skills, journal, crafting, map, player);
  ...
  public void draw(Graphics g) {
    if (!lock) {
      console.draw(
          Coords.getX("TR", console.getWidth() + 10),
          Coords.getY("BR", console.getHeight() + 20),
          g);
    ...
    if (cast.isOpenReq()) cast.draw(Coords.getX("CE", 0), Coords.getY("CE", 100));

    if (player.getTarget() != null) targetFrame.draw(Coords.getX("CE", 0), Coords.getY("TR", 0));

    if (charWin.isOpenReq()) charWin.draw(Coords.getX("CE", -250), Coords.getY("CE", -350));

    if (inventory.isOpenReq()) inventory.draw(Coords.getX("CE", -250), Coords.getY("CE", -300));

    if (skills.isOpenReq()) skills.draw(Coords.getX("CE", -500), Coords.getY("CE", -200));

    if (journal.isOpenReq()) journal.draw(Coords.getX("CE", -500), Coords.getY("CE", -200));

    if (map.isOpenReq()) map.draw(Coords.getX("CE", -250), Coords.getY("CE", -350), g);

    if (loot.isOpenReq()) loot.draw(Coords.getX("CE", -100), Coords.getY("CE", -100));

    if (trade.isOpenReq()) trade.draw(Coords.getX("CE", -150), Coords.getY("CE", -200));

    if (train.isOpenReq()) train.draw(Coords.getX("CE", -150), Coords.getY("CE", -200));

    if (dialogue.isOpenReq()) dialogue.draw(Coords.getX("CE", -150), Coords.getY("CE", -200));

    if (reading.isOpenReq()) reading.draw(Coords.getX("CE", -100), Coords.getY("CE", -100));

    if (igMenu.isOpenReq()) igMenu.draw(Coords.getX("CE", -100), Coords.getY("CE", -100));

    if (crafting.isOpenReq()) crafting.draw(Coords.getX("CE", -500), Coords.getY("CE", -200));

    if (save.isOpenReq()) save.draw(Coords.getX("CE", -100), Coords.getY("CE", -100), g);

    if (load.isOpenReq()) load.draw(Coords.getX("CE", -100), Coords.getY("CE", -100), g);

    if (settings.isOpenReq()) settings.draw(Coords.getX("CE", -100), Coords.getY("CE", -100));

    if (waitWin.isOpenReq()) waitWin.draw(Coords.getX("CE", -100), Coords.getY("CE", -100));

    if (info.isOpenReq())
      info.draw(
          Coords.getX("CE", 0) - (info.getScaledWidth() / 2),
          Coords.getY("CE", 0) - (info.getScaledHeight() / 2));

    // cursor.draw();
  }
  /** Draws UI pointers, like destination point or target point */
  public void drawPointners() {
    if (destination.isOpenReq()) destination.draw();
    if (target.isOpenReq()) target.draw();
  }
  /** Updates ui elements */
  public void update(GameContainer gc) {
    ...
    if (gw.isChangeAreaReq()) info.open(TConnector.getText("ui", "loadAreaInfo"));
    else if (!gw.isChangeAreaReq() && info.isOpenReq()) info.close();
  ...
  public boolean isMouseOver() {
    return bBar.isMouseOver()
        || igMenu.isMouseOver()
        || charFrame.isMouseOver()
        || console.isMouseOver()
        || inventory.isMouseOver()
        || skills.isMouseOver()
        || journal.isMouseOver()
        || loot.isMouseOver()
        || dialogue.isMouseOver()
        || trade.isMouseOver()
        || train.isMouseOver()
        || save.isMouseOver()
        || load.isMouseOver()
        || settings.isMouseOver()
        || crafting.isMouseOver()
        || charWin.isMouseOver()
        || map.isMouseOver()
        || waitWin.isMouseOver();
  }
  ...
  public void mouseReleased(int button, int x, int y) {
          ...
          Log.addSystem(
              "Move: "
                  ...
                  + gw.getArea()
                      .getMap()
                      .getTileId(
                          worldX / gw.getArea().getMap().getTileWidth(),
                          worldY / gw.getArea().getMap().getTileHeight(),
                          1)); // TEST LINE
        }
    ...
    if (button == Input.MOUSE_RIGHT_BUTTON) {
      ...
      if (!isMouseOver()) player.setTarget(null);
    }
  }
  ...
  public void keyReleased(int key, char c) {
    if (key == Input.KEY_F10) screenshotReq = true;
  }
  /**
   * UNUSED
   *
  ...
  private void keyDown(Input input) {
    if (!console.isFocused()) {
      if (input.isKeyDown(Input.KEY_W) && camera.getPos().y > -200) camera.up(Coords.getDis(32));
      if (input.isKeyDown(Input.KEY_S)
          && camera.getBRPos().y < (gw.getArea().getMapSize().height + Coords.getSize(200)))
        camera.down(Coords.getDis(32));
      if (input.isKeyDown(Input.KEY_A) && camera.getPos().x > -200) camera.left(Coords.getDis(32));
      if (input.isKeyDown(Input.KEY_D)
          && camera.getBRPos().x < (gw.getArea().getMapSize().width + Coords.getSize(200)))

...
class MapWindow extends InterfaceObject implements UiElement, MouseListener {
   ...
   */
  public MapWindow(GameContainer gc, Character player)
    ...
    playerMark =
        new Sprite(GConnector.getInput("sprite/hTarget.png"), "uiMapWindowPlayerMark", false);
  ...
  public void open(TiledMap map) {
    ...
    pmPos =
        new Position(
            ((mRenderStart.x) / mapScale) + (player.getPosition()[0]),
            ((mRenderStart.y) / mapScale) + (player.getPosition()[1]));
  }
  /* (non-Javadoc)
  ...
  private void renderMap(Graphics g) {
    ...
    map.render(
        (int) (renderStartX / mapScale),
        (int) (renderStartY / mapScale),
        fTileX,
        fTileY,
        (int) (mRenderWidth / mapScale),
        (int) (mRenderHeight / mapScale));
    if (pmPos.isIn(
        new Position(renderStartX, renderStartY),
        new Position((x / mapScale) + mRenderWidth, (y / mapScale) + mRenderHeight)))

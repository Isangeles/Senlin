 ...
 * @author Isangeles
 */
class BottomBar extends InterfaceObject
    implements UiElement, SaveElement, MouseListener, KeyListener {
  ...
  public BottomBar(
    ...
    questsB =
        new Button(
            GConnector.getInput("ui/button/buttonQuests.png"),
            "uiButtonQue",
            false,
            "",
            gc,
            TConnector.getText("ui", "questsBInfo"));
    characterB =
        new Button(
            GConnector.getInput("ui/button/buttonChar.png"),
            "uiButonChar",
            false,
            "",
            gc,
            TConnector.getText("ui", "characterBInfo"));
    inventoryB =
        new Button(
            GConnector.getInput("ui/button/buttonInventory.png"),
            "uiButtonInv",
            false,
            "",
            gc,
            TConnector.getText("ui", "inventoryBInfo"));
    craftingB =
        new Button(
            GConnector.getInput("ui/button/buttonCrafting.png"),
            "uiButtonCraft",
            false,
            "",
            gc,
            TConnector.getText("ui", "craftingBInfo"));
    skillsB =
        new Button(
            GConnector.getInput("ui/button/buttonSkills.png"),
            "uiButtonSkill",
            false,
            "",
            gc,
            TConnector.getText("ui", "skillsBInfo"));
    mapB =
        new Button(
            GConnector.getInput("ui/button/buttonMap.png"),
            "uiButtonMap",
            false,
            "",
            gc,
            TConnector.getText("ui", "mapBInfo"));
    menuB =
        new Button(
            GConnector.getInput("ui/button/buttonMenu.png"),
            "uiButtonQMenu",
            false,
            "",
            gc,
            TConnector.getText("ui", "menuBInfo"));
  ...
  public void mouseReleased(int button, int x, int y) {
    if (button == Input.MOUSE_LEFT_BUTTON) {
      if (menuB.isMouseOver() && !menu.isOpenReq()) menu.open();
      else if (menuB.isMouseOver() && menu.isOpenReq()) menu.close();

      if (characterB.isMouseOver() && !charWin.isOpenReq()) charWin.open();
      else if (characterB.isMouseOver() && charWin.isOpenReq()) charWin.close();

      if (inventoryB.isMouseOver() && !inventory.isOpenReq()) inventory.open();
      else if (inventoryB.isMouseOver() && inventory.isOpenReq()) inventory.close();

      if (craftingB.isMouseOver() && !crafting.isOpenReq()) crafting.open();
      else if (craftingB.isMouseOver() && crafting.isOpenReq()) crafting.close();

      if (skillsB.isMouseOver() && !skills.isOpenReq()) skills.open();
      else if (skillsB.isMouseOver() && skills.isOpenReq()) skills.close();

      if (questsB.isMouseOver() && !journal.isOpenReq()) journal.open();
      else if (questsB.isMouseOver() && journal.isOpenReq()) journal.close();

      if (mapB.isMouseOver() && !map.isOpenReq()) map.open(gw.getAreaMap());
      else if (mapB.isMouseOver() && map.isOpenReq()) map.close();
    }
  ...
  public void keyPressed(int key, char c) {
    if (key == Input.KEY_ESCAPE && !menu.isOpenReq()) menu.open();
    else if (key == Input.KEY_ESCAPE && menu.isOpenReq()) menu.close();

    if (key == Input.KEY_C && !charWin.isOpenReq()) charWin.open();
    else if (key == Input.KEY_C && charWin.isOpenReq()) charWin.close();

    if (key == Input.KEY_I && !inventory.isOpenReq()) inventory.open();
    else if (key == Input.KEY_I && inventory.isOpenReq()) inventory.close();

    if (key == Input.KEY_K && !skills.isOpenReq()) skills.open();
    else if (key == Input.KEY_K && skills.isOpenReq()) skills.close();

    if (key == Input.KEY_P && !crafting.isOpenReq()) crafting.open();
    else if (key == Input.KEY_P && crafting.isOpenReq()) crafting.close();

    if (key == Input.KEY_L && !journal.isOpenReq()) journal.open();
    else if (key == Input.KEY_L && journal.isOpenReq()) journal.close();

    if (key == Input.KEY_M && !map.isOpenReq()) map.open(gw.getAreaMap());
    else if (key == Input.KEY_M && map.isOpenReq()) map.close();
  ...
  public void keyReleased(int key, char c) {
    if (key == Input.KEY_1) sSlots.slots[0].click(false);
    if (key == Input.KEY_2) sSlots.slots[1].click(false);
    if (key == Input.KEY_3) sSlots.slots[2].click(false);
    if (key == Input.KEY_4) sSlots.slots[3].click(false);
    if (key == Input.KEY_5) sSlots.slots[4].click(false);
  }
  ...
  private class SkillSlots {
    ...
    public SkillSlot getDragged() {
      for (SkillSlot ss : slots) {
        if (ss.isSkillDragged()) return ss;
      }
      return null;
    }

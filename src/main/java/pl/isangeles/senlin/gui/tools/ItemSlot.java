...
class ItemSlot extends Slot implements MouseListener {
  ...
  public boolean insertContent(SlotContent item) {
    if (!isFull()) {
      if (Item.class.isInstance(item)) {
        if (isEmpty()) return super.content.add((Item) item);
        else {
    ...
    if (!isFull()) {
      if (isEmpty()) {
        ...
        if (items.size() > 0) {
          ...
          if (items.size() > item.getMaxStack()) return false;
          else return this.content.addAll(items);
        } else return false;
      } else {
        SlotContent itemIn = this.content.get(0);
        for (SlotContent item : content) {
          if (!item.getId().equals(itemIn.getId())) return false;
        }
        if (content.size() > itemIn.getMaxStack() - this.content.size()) return false;
        else return this.content.addAll(content);
      }
    } else return false;
  }
  ...
  public boolean isItemDragged() {
    if (!isEmpty()) return content.get(0).getTile().isDragged();
    else return false;
  }

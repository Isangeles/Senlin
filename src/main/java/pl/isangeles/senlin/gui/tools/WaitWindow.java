...
class WaitWindow extends InterfaceObject implements UiElement, KeyListener, MouseListener {
   ...
   */
  public WaitWindow(GameContainer gc, GameWorld gw)
  ...
  public void draw(float x, float y) {
    ...
    if (rest)
      ttf.drawString(
          super.getCenter(false).x, super.getTR(false).y, TConnector.getText("ui", "waitWinTitle"));
    else
      ttf.drawString(
          super.getCenter(false).x, super.getTR(false).y, TConnector.getText("ui", "waitWinRest"));
    ...
    waitB.draw(
        super.getCenter(false).x - waitB.getScaledWidth() / 2,
        super.getCenter(false).y + getDis(40),
        false);
    waitTimeF.draw(
        super.getCenter(false).x - getSize(35f), super.getCenter(false).y - getSize(25f), false);
  }
  /**
   * Opens window
   *
  ...
  public void open(boolean rest) {
    openReq = true;
    this.rest = rest;
    if (rest) waitB.setLabel(TConnector.getText("ui", "waitWinRest"));
    else waitB.setLabel(TConnector.getText("ui", "waitWinWait"));
  }
  /* (non-Javadoc)
  ...
  public void mouseReleased(int button, int x, int y) {
    if (button == Input.MOUSE_LEFT_BUTTON) {
      if (hPlusB.isMouseOver()) waitTimeH += 1;
      if (hMinusB.isMouseOver() && waitTimeH > 0) waitTimeH -= 1;
  ...
  public void keyReleased(int key, char c) {
    if (key == Input.KEY_T && !openReq) open(false);
    else if (key == Input.KEY_ESCAPE || (key == Input.KEY_T && openReq)) close();
  }
}

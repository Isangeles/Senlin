...
public class SkillSlot extends Slot implements MouseListener {
  ...
  public void click(boolean clicked) {
    if (!isEmpty()) skillInSlot.getTile().click(clicked);
  }
  /** Uses skill in slot */
  ...
  public boolean isActive() {
    if (skillInSlot != null) return skillInSlot.isReady();
    else return false;
  }
  /**
  ...
  public boolean isSkillDragged() {
    if (!isEmpty()) return skillInSlot.getTile().isDragged();
    else return false;
  }
  ...
  public void mousePressed(int button, int x, int y) {
    ...
    if (!isEmpty() && isMouseOver()) skillInSlot.getTile().click(true);
  }

  @Override
  public void mouseReleased(int button, int x, int y) {
    if (!isEmpty()) {
      if (isMouseOver() && button == Input.MOUSE_LEFT_BUTTON) useSkill();
      else skillInSlot.getTile().click(false);
    }
  }
}

...
/**
 * Class for loot window TODO disable items dragging
 *
 * @author Isangeles
 */
class LootWindow extends InterfaceObject implements UiElement, MouseListener, KeyListener {
   ...
   */
  public LootWindow(GameContainer gc, Character player)
    ...
    takeAll =
        new Button(GConnector.getInput("button/buttonS.png"), "uiLootB1S", false, "Take all", gc);
    ...
    if (openReq == false) {
      ...
      if (lootingChar.getRangeFrom(lootedChar.getPosition()) < 40) {
        if (!lootedChar.getInventory().isLocked()
            || lootedChar.getInventory().unlock(lootingChar)) {
          slotsP.clear();
          loadLoot();
          openReq = true;
          focus = true;
          return CharacterOut.SUCCESS;
        } else return CharacterOut.LOCKED;
      } else {
  ...
  public void mousePressed(int button, int x, int y) {
    ...
    if (slot != null) slot.dragged(false);
  }

...
/**
 * Class for targetable objects frame TODO fix experience bar on resolutions different then default
 *
 * @author Isangeles
 */
class TargetFrame extends InterfaceObject {
   ...
   */
  public TargetFrame(GameContainer gc, Targetable object)
  ...
  public void draw(float x, float y) {
    ...
    target.getPortrait().draw(x + getDis(40), y + getDis(9), 95f, 130f, false);
    ...
    for (Effect effect : effects) {
      ...
      icon.draw(
          x + getDis(34) + (icon.getScaledWidth() * column),
          y + getDis(142) + ((icon.getScaledHeight() + getDis(15)) * row),
          Stopwatch.timeFromMillis(effect.getTime()),
          false);
      column++;
      if (column == 6) {
        row++;

...
class TrainingWindow extends InterfaceObject implements UiElement, MouseListener {
   ...
   * @throws IOException
   */
  public TrainingWindow(GameContainer gc, Character player)
    ...
    train =
        new Button(
            ...
            "uiTrainButton",
            false,
            TConnector.getText("ui", "trainWinTrain"),
            gc);
    exit =
        new Button(
            GConnector.getInput("button/buttonS.png"),
  ...
  public void mouseReleased(int button, int x, int y) {
    if (button == Input.MOUSE_LEFT_BUTTON) {
      if (exit.isMouseOver()) close();
      if (train.isMouseOver()) {
        if (selectedTrain != null) {
          try {
            if (selectedTrain.teach(player))
              Log.addInformation(
                  TConnector.getText("ui", "trainWinTrain") + ": " + selectedTrain.getName());
            else
              Log.addInformation(
                  TConnector.getText("ui", "trainWinFail") + ": " + selectedTrain.getName());

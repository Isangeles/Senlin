...
class InventoryMenu extends InterfaceObject implements UiElement, SaveElement, MouseListener {
   ...
   */
  public InventoryMenu(GameContainer gc, Character player)
  ...
  public void draw(float x, float y) {
    ...
    // textTtf.drawString(x+getDis(390), y+getDis(215), TConnector.getText("ui", "iMenuGold") + ": "
  ...
  public void mouseReleased(int button, int x, int y) {
    if (button == Input.MOUSE_LEFT_BUTTON) {
      if (getDragged() != null) {
        ...
        if (slotOver != null) {
          if (eqSlots.contains(draggedSlot)) eqSlots.removeFromEq(draggedSlot);
        ...
        if (eqSlots.getMouseOverSlot() != null) {
          ...
          if (Equippable.class.isInstance(draggedSlotContent)) {
            ...
            else Log.addWarning(TConnector.getText("ui", "equipReqFail"));
          }
          return;
        }
    ...
    for (Slot slot : slots.getAllSlots()) {
      if (!slot.isEmpty()) {
        for (SlotContent content : slot.getContent()) {
          if (!itemsIn.contains(content)) slot.removeContent(content);
        }
      }
    }
  ...
  private ItemSlot getDragged() {
    ...
    if (nSlot != null) return nSlot;

    for (ItemSlot slot : eqSlots.slotsTable) {
      if (slot.isItemDragged()) return slot;
    }
    return null;
  }
  /**
  ...
  private class EquipmentSlots {
     ...
     * @throws FontFormatException
     */
    public EquipmentSlots(GameContainer gc)
        throws SlickException, IOException, FontFormatException {
      ...
      slotsTable =
          new ItemSlot[] {
            feet, hands, offhand, chest, head, weapon, finger, secFinger, neck, artifact
          };
    }
    /**
    ...
    public boolean setEqItem(Equippable eqItem, ItemSlot eqSlot) {
      try {
        if (!isCompatible(eqItem, eqSlot)) return false;
      ...
      if ((eqSlot == feet || eqSlot == hands || eqSlot == chest || eqSlot == head)
          && Armor.class.isInstance(eqItem)) return player.equipItem(eqItem);
      if ((eqSlot == finger || eqSlot == neck || eqSlot == artifact)
          && Trinket.class.isInstance(eqItem)) return player.equipItem(eqItem);
    ...
    public ItemSlot getMouseOverSlot() {
      for (ItemSlot slot : slotsTable) {
        if (slot.isMouseOver()) return slot;
      }
    ...
    public void removeFromEq(ItemSlot slot) {
      for (ItemSlot is : slotsTable) {
        if (slot == is) {
          ...
          if (Equippable.class.isInstance(item)) player.unequipp((Equippable) item);
        }
      }
    }
    /**
    ...
    private boolean isCompatible(Equippable item, ItemSlot slot) {
      if (Armor.class.isInstance(item)) {
        if (item.type() != Armor.FEET && slot == feet) return false;
        if (item.type() != Armor.HANDS && slot == hands) return false;
        if (item.type() != Armor.OFFHAND && slot == offhand) return false;
        if (item.type() != Armor.CHEST && slot == chest) return false;
        if (item.type() != Armor.HEAD && slot == head) return false;
      }
      if (!Weapon.class.isInstance(item) && (slot == weapon || slot == offhand)) return false;

      if (Trinket.class.isInstance(item)) {
        if (item.type() != Trinket.FINGER && slot == finger) return false;
        if (item.type() != Trinket.FINGER && slot == secFinger) return false;
        if (item.type() != Trinket.NECK && slot == neck) return false;
        if (item.type() != Trinket.ARTIFACT && slot == artifact) return false;
      }
    ...
    public ItemSlot getSlotFor(Equippable item) {
      if (Weapon.class.isInstance(item)) {
        ...
        else if (offhand.isEmpty()) return offhand;
        else return weapon;
      }
      ...
      if (Trinket.class.isInstance(item)) {
        switch (item.type()) {
          case Trinket.ARTIFACT:
            return artifact;
          case Trinket.FINGER:
            if (finger.isEmpty()) return finger;
            else if (secFinger.isEmpty()) return secFinger;
            else return finger;
          case Trinket.NECK:
            return neck;
        }

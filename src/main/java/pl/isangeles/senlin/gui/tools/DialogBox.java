...
class DialogBox extends InterfaceObject implements UiElement, MouseListener {
   ...
   */
  public DialogBox(GameContainer gc, GameWorld gw)
    ...
    optionsUp =
        new Button(GConnector.getInput("button/buttonUp.png"), "uiDialogBoxOUp", false, "", gc);
    optionsDown =
        new Button(GConnector.getInput("button/buttonDown.png"), "uiDialogBoxODown", false, "", gc);
  ...
  public void draw(float x, float y) {
    ...
    for (int i = 0; i < options.size(); i++) {
      options
          .get(i)
          .draw(x + getDis(15), (y + getDis(360)) + (i * options.get(i).getHeight()), false);
    }
  }
  ...
  public void close() {
    openReq = false;
    if (interlocutorA != null) interlocutorA.getSignals().stopTalking();
    if (interlocutorB != null) interlocutorB.getSignals().stopTalking();
  ...
  private class DialogueOption extends Button {
     ...
     * @throws IOException
     */
    public DialogueOption(GameContainer gc)
        throws SlickException, FontFormatException, IOException {
      super(
          GConnector.getInput("ui/background/dialogueOptionBG.png"),
          "uiDialogueBoxOptionBg",
          false,
          "",
          gc);
    }
    ...
    public void draw(float x, float y, boolean scaledPos) {
      ...
      if (option != null) super.drawString(option.getTextFor(interlocutorA), ttf, false);
    }
      ...
      for (int i = 0; i < visibleTexts.size(); i++) {
        ...
        if (i == 0) {
          if (textsLeft.contains(text))
            text.draw(
                super.x, (super.y + super.getScaledHeight() - getDis(25)) - text.getTextHeight());
          if (textsRight.contains(text))
            text.draw(
                super.getTR(scaledPos).x - text.getTextWidth(),
                (super.y + super.getScaledHeight() - getDis(25)) - text.getTextHeight());
        } else if (i > 0) {
          ...
          if (textsRight.contains(text))
            text.draw(
                super.getTR(scaledPos).x - text.getTextWidth(),
                (prevText.getPosition().y - getDis(10)) - text.getTextHeight());
        }
      }
    }
    /**

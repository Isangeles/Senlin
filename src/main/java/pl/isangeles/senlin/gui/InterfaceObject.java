...
public abstract class InterfaceObject extends Image {
   ...
   */
  protected InterfaceObject(String pathToTex, GameContainer gc)
   ...
   * @throws FileNotFoundException
   */
  protected InterfaceObject(String pathToTex, int moaWidth, int moaHeight, GameContainer gc)
      throws SlickException, FileNotFoundException {
    super(pathToTex);
    this.gc = gc;
    moa =
        new MouseOverArea(
            gc, this, (int) Coords.getX("BR", 0), (int) Coords.getY("BR", 0), moaWidth, moaHeight);
  }
  /**
   ...
   * @throws SlickException
   */
  protected InterfaceObject(Image image, GameContainer gc, String textForInfo)
   ...
   * @throws FileNotFoundException
   */
  protected InterfaceObject(Image image, int moaWidth, int moaHeight, GameContainer gc)
      throws FileNotFoundException {
    super(image);
    this.gc = gc;
    moa =
  ...
  protected InterfaceObject(
      Image image, String textForInfo, int moaWidth, int moaHeight, GameContainer gc)
  ...
  /**
   * Constructor for interface object without info window, implicitly scales object for current
   * resolution
   *
   ...
   * @throws IOException
   */
  protected InterfaceObject(InputStream fileInput, String ref, boolean flipped, GameContainer gc)
  ...
  /**
   * Constructor for interface object with info window, implicitly scales object for current
   * resolution
   *
   * @param fileInput in - The input stream to read the image from
   * @param ref The name that should be assigned to the image
   ...
   */
  protected InterfaceObject(
      InputStream fileInput, String ref, boolean flipped, GameContainer gc, String textForInfo)
      throws SlickException, IOException, FontFormatException {
  ...
  /**
   * Constructor for interface object with info window and custom MOA size, implicitly scales object
  ...
  /** Draws object on current position (position is scaled) */
  @Override
  public void draw() {
  ...
  /**
   * Draws object with specific width and height All specified parameters are scaled to current
   * resolution
   *
   * @param x Position on x axis
   * @param y Position on y axis
  ...
  public Position atCenter() {
    return new Position(
        Coords.getX("CE", 0) - (getScaledWidth() / 2),
        Coords.getY("CE", 0) - (getScaledHeight() / 2));
  }
  /**
  ...
  public void setInfo(String info) {
    if (isInfo) this.info.setText(info);
  }
  /**
  ...
  protected void drawString(String text, TrueTypeFont ttf, boolean scaledPos) {
    ...
    */
    ttf.drawString(
        getCenter(scaledPos).x - textWidth / 2, getCenter(scaledPos).y - textHeight / 2, text);
  }
  /**
  ...
  protected void drawString(String text, TrueTypeFont ttf, Color color, boolean scaledPos) {
    ...
    */
    ttf.drawString(
        getCenter(scaledPos).x - textWidth / 2,
        getCenter(scaledPos).y - textHeight / 2,
        text,
        color);
  }
  /**
  ...
  protected Position getTR(boolean scaledPos) {
    ...
    if (isCustomSize()) return new Position((int) (posx + width), (int) posy);
    else return new Position((int) (posx + getScaledWidth()), (int) posy);
  }
  /**
  ...
  protected Position getBR(boolean scaledPos) {
    ...
    if (isCustomSize()) return new Position((int) (posx + width), (int) (posy + height));
    else return new Position((int) (posx + getScaledWidth()), (int) (posy + getScaledHeight()));
  }
  /**
  ...
  protected Position getBL(boolean scaledPos) {
    ...
    if (isCustomSize()) return new Position((int) posx, (int) (posy + height));
    else return new Position((int) posx, (int) (posy + getScaledHeight()));
  }
  /**
  ...
  protected Position getCenter(boolean scaledPos) {
    ...
    if (isCustomSize()) return new Position(posx + (width / 2), posy + (height / 2));
    else return new Position(posx + (getScaledWidth() / 2), posy + (getScaledHeight() / 2));
  }
  /**
  ...
  private boolean isCustomSize() {
    return ((width != getWidth() && width != getScaledWidth())
        || (height != getHeight() && height != getScaledHeight()));
  }
}

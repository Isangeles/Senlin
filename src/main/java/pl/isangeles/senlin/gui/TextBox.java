...
public class TextBox extends InterfaceObject implements MouseListener {
  ...
  public void draw(float x, float y, float width, float height, boolean scaledPos) {
    ...
    down.draw(
        getBR(scaledPos).x - down.getScaledWidth(),
        getBR(scaledPos).y - down.getScaledHeight(),
        scaledPos);
  ...
  public boolean contains(String rawText) {
    for (TextBlock text : texts) {
      if (text.getText().equals(rawText)) return true;
    }
  ...
  protected Position getTR(boolean scaledPos) {
    return new Position(
        (int) (super.getTR(scaledPos).x - up.getScaledWidth()) - getDis(10),
        super.getTR(scaledPos).y);
  }
  /**
   * Sets list of text blocks that should be displayed in box method checks each block height and
   * add them to list only if height of all blocks in list does not exceed text box height
   */
  private void setVisibleTexts() {
    ...
    for (int i = firstIndex; i >= 0; i--) {
      ...
      // //TEST LINE
      if (textsHeight > height) return;
      else visibleTexts.add(text);
    }
  }
}

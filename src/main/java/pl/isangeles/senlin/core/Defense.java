...
public class Defense {
  ...
  public void handleSkill(Skill skill) {
    ...
    if (Attack.class.isInstance(skill)) {
      ...
      if (rng.nextDouble() + owner.getAttributes().getDodge() >= 1f) {
        ...
        damage -=
            owner.getInventory().getArmorRating()
                - owner.getAttributes().getResistances().getResistanceFor(attack.getEffectType());
  ...
  public void handleEffect(Effect effect) {
    ...
    else Log.addCombat(owner.getName() + ":" + TConnector.getText("ui", "logResi"));
  }
  /**
   * Handles effects
   *

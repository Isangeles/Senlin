...
public class Character implements Targetable, ObjectiveTarget, SaveElement {
  ...
  public Character(
    ...
    live = true;
    if (staticAvatar) avatar = new StaticAvatar(this, gc, spritesheet);
    else avatar = new DynamicAvatar(this, gc, spritesheet);
  ...
  public Character(
    ...
    this(
        ...
        atributes,
        portrait,
        race.getDefaultSpritesheet(sex),
        false,
        gc);
  }
  /**
  ...
  public boolean setDefaultPosition(Position pos) {
    if (currentArea == null
        || pos.isIn(
            new Position(0, 0),
            new Position(currentArea.getMapSize().width, currentArea.getMapSize().height))) {
  ...
  public boolean equipItem(Equippable item) {
    if (item != null && item.getEquipReqs().isMetBy(this)) return inventory.equip(item);
    else return false;
  }
  /**
  ...
  public void setTrade() {
    ...
    if (!hasDefD) dialogues.add(DialoguesBase.getDefaultDialogue());
  ...
  public CharacterOut update(int delta) {
    ...
    if (agony && hp.getValue() >= 10) agony = false;
    else if (hp.getValue() < 10 && hp.getValue() > 0) agony = true;
    ...
    if (signals.containsKey(CharacterSignal.FIGHTING)) {
      ...
      if (target.isLive()) out = useSkillOn(target, abilities.get("autoA"));
      else signals.remove(CharacterSignal.FIGHTING);
    }
  ...
  public void moveTo(Targetable target, int maxRange) {
    ...
    Position hitBoxStart =
        new Position(target.getPosition()[0] - maxRange, target.getPosition()[1] - maxRange);
    Position hitBoxEnd =
        new Position(target.getPosition()[0] + maxRange, target.getPosition()[1] + maxRange);
  ...
  public boolean isMove() {
    if (position.x != destPoint.x || position.y != destPoint.y) return true;
    else return false;
  }
  /**
  ...
  public int getHit() {
    ...
    if (rng.nextDouble() + attributes.getMainHitChance() >= 1.0f) {
      ...
      } else mainHit += modifiers.getDmgBonusFor(WeaponType.FIST);
    } else mainHit = -1; // means miss

    if (mainWeapon == null && offWeapon != null) mainHit = 0;
    ...
    if (inventory.isDualwield() || offWeapon != null || (mainWeapon == null && offWeapon == null)) {
      if (rng.nextDouble() + attributes.getOffHitChance() >= 1.0f) {
        ...
        } else offHit += modifiers.getDmgBonusFor(WeaponType.FIST);
      } else offHit = -1; // means miss
    }
  ...
  public Attitude getAttitudeTo(Character character) {
    if (character == null) return attitude;
    else if (!isLive()) return Attitude.DEAD;
  ...
  public Guild getGuild() {
    if (guild == null) return GuildsBase.getGuild("none");
    else return guild;
  }
  /**
  ...
  public boolean isNearby(Targetable object) {
    if (this.getRangeFrom(object.getPosition()) <= 400) return true;
    else return false;
  }

  public boolean isNearby(int[] pos) {
    if (getRangeFrom(pos) <= 400) return true;
    else return false;
  }
  /* (non-Javadoc)
  ...
  public boolean isTalking() {
    if (signals.get(CharacterSignal.TALKING) != null) return true;
    else return false;
  }

  public boolean isFighting() {
    if (signals.get(CharacterSignal.FIGHTING) != null) return true;
    else return false;
  }
  /* (non-Javadoc)
  ...
  public boolean addSkill(Skill skill) {
    if (skill != null) return abilities.add(skill);
    else return false;
  }
  /* (non-Javadoc)
  ...
  public void startQuest(Quest quest) {
    ...
    } else Log.addSystem("char_startQuest_fail-msg//fail to add quest to list");
  }
  /* (non-Javadoc)
  ...
  public CharacterOut useSkill(Skill skill) {
    if (live && !agony && skill != null && abilities.contains(skill)) {
      ...
      if (out.isSuccess()) {
        sCaster.cast(skill);
        return CharacterOut.SUCCESS;
      } else return CharacterOut.UNABLE;
    } else return CharacterOut.UNABLE;
  }
  /**
   * Activates specified skill(only if character know this skill)
  ...
  public void speak(String what) {
    avatar.speak(what);
    if (this.isNearby(Global.getPlayer())) Log.addSpeech(name, what);
  }
  /**
  ...
  public Element getSave(Document doc) {
    ...
    if (currentArea != null) positionE.setAttribute("area", currentArea.getId());
    else positionE.setAttribute("area", "none");
   ...
   * @return Dialogue object for specified character or default dialogue from base if no proper
   *     dialogue was found
   */
  private Dialogue getDialogueFor(Character character) {
    ...
    for (Dialogue dialogue : dialogues) {
      if (!dialogue.hasReqs() && !dialogue.getId().equals("default")) return dialogue;
    }

    for (Dialogue dialogue : dialogues) {
      if (!dialogue.hasReqs()) return dialogue;
    }

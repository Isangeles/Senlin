...
public class Misc extends Item {
  ...
  public boolean use(Targetable user, Targetable target) {
    ...
    if (out && disposable) user.getInventory().remove(this);
    return out;
  }
  /**
  ...
  protected String getInfo() {
    String fullInfo =
        name + System.lineSeparator() + info + System.lineSeparator() + "Value: " + value;
    return fullInfo;
  }
  /* (non-Javadoc)
  ...
  protected ItemTile buildIcon(GameContainer gc)
    ...
    try {
      ...
      System.err.println(
          "item_icon_build_fail_msg-//" + e.getMessage() + ", img_file_name:" + imgName);
      return new ItemTile(GBase.getImage("errorIcon"), gc, this.getInfo());
    }
  }
}

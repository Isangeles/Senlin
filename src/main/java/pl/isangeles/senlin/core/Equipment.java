...
public class Equipment implements SaveElement {
  ...
  public boolean equip(Equippable item) {
    ...
    if (Armor.class.isInstance(item)) return setArmor((Armor) item);
    else if (Weapon.class.isInstance(item)) return setWeapon((Weapon) item);
  ...
  public int getArmorRat() {
    ...
    if (boots != null) rating += boots.getArmorRat();
    if (gloves != null) rating += gloves.getArmorRat();
    if (chest != null) rating += chest.getArmorRat();
    if (helmet != null) rating += helmet.getArmorRat();
    if (shield != null) rating += shield.getArmorRat();
  ...
  public List<Equippable> getAll() {
    ...
    if (weaponMain != null) eq.add(weaponMain);
    if (weaponSec != null) eq.add(weaponSec);
    if (boots != null) eq.add(boots);
    if (gloves != null) eq.add(gloves);
    if (shield != null) eq.add(shield);
    if (chest != null) eq.add(chest);
    if (helmet != null) eq.add(helmet);
    if (ring != null) eq.add(ring);
    if (ringSec != null) eq.add(ringSec);
    if (amulet != null) eq.add(amulet);
    if (artifact != null) eq.add(artifact);
    return eq;
  }
  /**
  ...
  public boolean isDualwield() {
    return (weaponMain != null
        && weaponSec != null); // || (weaponMain == null && weaponSec == null);
  }
  /**

...
public class Dialogue implements SaveElement {
  public static final String NAME_MACRO = "[$]name"; // regex
  ...
  public void answerOn(Answer answer) {
    if (dialogueTarget != null) dialogueTarget.getQTracker().check(answer);

    if (!answer.isEnd()) {
      currentStage = getPart(answer.getTo());
      if (dialogueTarget != null) dialogueTarget.getQTracker().check(currentStage);
    } else reset();
  }
  /** Resets dialogue */
  public void reset() {
  ...
  public void addOption(Answer option) {
    if (parts.size() >= 1) {
      if (parts.get(0) != null) parts.get(0).addAnswer(option);
    }
  }
  /**
   * Returns dialogue ID
  ...
  private DialoguePart getPart(String ordinalId) {
    ...
    for (DialoguePart dp : parts) {
      if (dp.getOrdinalId().equals(ordinalId) && !dp.hasReq()) return dp;
    }

    for (DialoguePart dp : parts) {
      if (dp.getId().equals("error01")) return dp;
    }

...
public class DialoguePart implements ObjectiveTarget {
   ...
   */
  public DialoguePart(
      String id, String ordinalId, boolean start, List<Requirement> req, List<Answer> answers) {
    this.id = id;
  ...
  public List<Answer> getAnswersFor(Character dTarget) {
    ...
    for (Answer answer : this.answers) {
      if (answer.getReqs().isMetBy(dTarget)) answers.add(answer);
    }
    return answers;
  }
  /**
  ...
  public void modify(Character charA, Character charB) {
    if (transfer != null) transfer.exchange(charA, charB);
  ...
  public boolean hasReq() {
    if (reqs != null && reqs.size() > 0) return true;
    else return false;
  }
}

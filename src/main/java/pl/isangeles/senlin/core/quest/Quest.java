...
public class Quest implements ScrollableContent, SaveElement {
  ...
  public Quest(
      String id,
      List<String> flagsOnStart,
      List<String> flagsOffStart,
      List<String> flagsOnEnd,
      List<String> flagsOffEnd,
      List<Stage> stages) {
    this.id = id;
    this.stages = stages;
    ...
    for (Stage stage : stages) {
      if (stage.isStartStage()) currentStage = stage;
    }
  ...
  public void setStage(String stageId) {
    for (Stage stage : stages) {
      if (stage.getId().equals(stageId)) currentStage = stage;
    }
  }
  /**
  ...
  public boolean hasFlag() {
    if (!flagsOnStart.isEmpty()) return true;

    for (Stage stage : stages) {
      if (stage.hasFlag()) return true;
    }

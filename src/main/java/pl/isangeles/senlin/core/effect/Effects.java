...
public class Effects extends ArrayList<Effect> implements SaveElement {
        ...
        if (!hasEffect(id)) {
          ...
          for (Effect effect : get(id)) {
            if (effect.getSource() != null
                ...
                && effect.getSource().getSerialId().equals(source.getSerialId())) {
              add(source.getEffect(id));
              break;
            }
          }
  ...
  public void removeAllFrom(EffectSource source) {
    for (Effect effect : this) {
      if (effect.getSource() == source) effectsToRemove.add(effect);
    }
  }
  /**
  ...
  public boolean hasEffect(Effect effect) {
    for (Effect e : this) {
      if (e.getId().equals(effect.getId())) return true;
    }
    return false;
  }
  /**
  ...
  public boolean hasEffect(String effectId) {
    for (Effect effect : this) {
      if (effect.getId().equals(effectId)) return true;
    }
    return false;
  }
  /**
  ...
  public boolean hasEffectFrom(String effectId, String sourceEffectId) {
    for (Effect effect : this) {
      if (effect.getId().equals(effectId)) {
        if (effect.getSource() != null) {
          if (effect.getSource().getSerialId().equals(sourceEffectId)) return true;
        }
      }
    }
    return false;
  ...
  public List<Effect> get(String effectId) {
    ...
    for (Effect effect : this) {
      if (effect.getId().equals(effectId)) effects.add(effect);
    }
    return effects;
  }
  /* (non-Javadoc)

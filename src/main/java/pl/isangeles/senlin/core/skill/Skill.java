...
public abstract class Skill implements SlotContent, SaveElement, EffectSource {
  ...
  public int getCooldown() {
    if (isMagic()) return (int) (cooldown / owner.getAttributes().getConcentration());
    else return (int) (cooldown / owner.getAttributes().getHaste());
  }
  /**
   * Returns skill ID
   *
  ...
  public boolean isMagic() {
    for (Requirement req : useReqs) {
      if (ManaRequirement.class.isInstance(req)) return true;
    }
    return false;
  }
  ...
  protected void setTile(GameContainer gc) throws SlickException, IOException, FontFormatException {
    this.tile =
        new SkillTile(
            GConnector.getInput("icon/skill/" + imgName), "skillTile", false, gc, getInfo());
  }
  /**

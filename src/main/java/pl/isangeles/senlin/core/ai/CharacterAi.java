...
public class CharacterAi {
    ...
    for (Character npc : aiNpcs) {
      ...
      if (area != null) {
        ...
        if (rng.nextInt(100) == 1) {
          moveAround(npc);
          if (npc.getRace() == Race.HUMAN) saySomething(npc, "idle", true);
        }

        Targetable target = npc.getTarget();

        if (npc.getSignals().get(CharacterSignal.FIGHTING) == null
            && npc.getSignals().get(CharacterSignal.FOLLOWING) == null) {
          // npc.moveTo(npc.getDefaultPosition());
        } else if (target != null
            && npc.getSignals().get(CharacterSignal.FIGHTING) == target
            && !npc.isNearby(target)) // checks if combat target is out of range
        {
          npc.setTarget(null);
        ...
        if (!nearbyChars.isEmpty()) {
          for (Character nearbyChar : nearbyChars) {
            if (npc.getAttitudeTo(nearbyChar) == Attitude.HOSTILE
                || nearbyChar.getAttitudeTo(npc) == Attitude.HOSTILE) {
      ...
      if (!npc.isLive()) deadNpcs.add(npc);
      if (area == null) lostNpcs.add(npc);
    }
  ...
  private void moveAround(Character npc) {
    if (npc.getTarget() == null) {
      switch (rng.nextInt(8)) {
        case 0:
          if (npc.getAvatar().getDirection() != Coords.UP) npc.moveBy(0, -4);
          break;
        case 1:
          if (npc.getAvatar().getDirection() != Coords.RIGHT) npc.moveBy(4, 0);
          break;
        case 2:
          if (npc.getAvatar().getDirection() != Coords.DOWN) npc.moveBy(0, 4);
          break;
        case 3:
          if (npc.getAvatar().getDirection() != Coords.LEFT) npc.moveBy(-4, 0);
          break;
        default:
          return;
      }
  ...
  private void saySomething(Character who, String what, boolean random) {
    ...
    } else who.speak(TConnector.getRanomText(who.getRace().getRandomTextId(what)));
  }
  /**

  ...
  public void addHours(int hours) {
    if (hours > 0) {
      for (int i = 1; i <= hours; i++) {
        time += 60000;
        if (Stopwatch.toMin(time) > 23) time = 0l;
      }
    } else {
      for (int i = 1; i <= -hours; i++) {
        time -= 60000;
        if (Stopwatch.toMin(time) < 0) time = Stopwatch.min(23);
      }
    }
  }
  /**
  ...
  public void addMinutes(int minutes) {
    if (minutes > 0) {
      for (int i = 0; i < minutes; i++) {
        time += 1000;
        if (Stopwatch.toSec(time) > 1380) time = 0l;
      }
    } else {
      for (int i = 0; i < -minutes; i++) {
        time -= 1000;
        if (Stopwatch.toSec(time) < 0) time = Stopwatch.min(23);
      }
    }
  }
  /**

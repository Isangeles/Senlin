...
public class Attributes {
  ...
  public Attributes nagative() {
    return new Attributes(
        -strenght.value, -constitution.value, -dexterity.value, -intelligence.value, -wisdom.value);
  }
   ...
   * @return False if specified attributes have any attribute greater, true if all attributes are
   *     lower or equal
   */
  public boolean compareTo(Attributes otherAtt) {
    if (strenght.getValue() < otherAtt.getStr()
        || constitution.getValue() < otherAtt.getCon()
        || dexterity.getValue() < otherAtt.getDex()
        || intelligence.getValue() < otherAtt.getInt()
        || wisdom.getValue() < otherAtt.getWis()) return false;

    if (strenght.getValue() >= otherAtt.getStr()
        && constitution.getValue() >= otherAtt.getCon()
        && dexterity.getValue() >= otherAtt.getDex()
        && intelligence.getValue() >= otherAtt.getInt()
        && wisdom.getValue() >= otherAtt.getWis()) return true;
  ...
  public String getInfo() {
    ...
    if (strenght.value != 0) info += TConnector.getText("ui", "attStrName") + ":" + strenght.value;
    ...
    if (wisdom.value != 0) info += TConnector.getText("ui", "attWisName") + ":" + wisdom.value;

...
public class ScriptProcessor {
    ...
    if (!script.isFinished() && !script.isWaiting()) {
      try {
        ...
        if (!script.hasNext()) {
          script.used();
          script.restart();
          if (checkEndCondition(script)) script.finish();
        }
        ...
        Log.addSystem(
            "ssp: "
                + script.getName()
                + " processing fail - corrupted at line:"
                + script.getActiveIndex());
      }
    }
    return out;
   ...
   * @throws IndexOutOfBoundsException
   */
  private boolean checkConditions(Script script, List<String> ifOrCode)
      throws IndexOutOfBoundsException {
    boolean out = false;
  ...
  private boolean checkEndCondition(Script script) throws IndexOutOfBoundsException {
    ...
    } else if (statement.equals("now;")) out = true;

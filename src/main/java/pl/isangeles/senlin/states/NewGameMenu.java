...
public class NewGameMenu extends BasicGameState {
  ...
  public void init(GameContainer container, StateBasedGame game) throws SlickException {
    try {
      ...
      conS =
          new Switch(container, "Constitution", 1, attrPoints, TConnector.getText("ui", "conInfo"));
      ...
      genS =
          new TextSwitch(
              container,
              TConnector.getText("ui", "genName"),
              new String[] {
                TConnector.getText("ui", "genMale"), TConnector.getText("ui", "genFemale")
              });
      attrPointsF =
          new PointsField(
              GConnector.getInput("field/ptsFieldBG.png"),
              "fieldAP",
              false,
              attrPoints,
              "Points",
              container,
              TConnector.getText("ui", "attPtsInfo"));
      ...
      nextPortraitB =
          new Button(GConnector.getInput("button/buttonNext.png"), "buttNP", false, "", container);
      prevPortraitB =
          new Button(GConnector.getInput("button/buttonBack.png"), "buttBP", false, "", container);

      nameI =
          new TextInput(
              GConnector.getInput("field/textFieldBG.png"), "fieldName", false, container);

      nextB =
          new Button(GConnector.getInput("button/buttonNext.png"), "buttN", false, "", container);
      backB =
          new Button(GConnector.getInput("button/buttonBack.png"), "buttB", false, "", container);
  ...
  public void update(GameContainer container, StateBasedGame game, int delta)
    ...
    if (nameI.getText() != null && attrPoints.getValue() == 0) nextB.setActive(true);
  ...
  public void mouseReleased(int button, int x, int y) {
    if (nextPortraitB.isMouseOver() && imgId < portraits.size() - 1) imgId++;
    else if (prevPortraitB.isMouseOver() && imgId > 0) imgId--;

    if (nextB.isMouseOver() && nextB.isActive()) {
      gameWorldReq = true;
    }

    if (backB.isMouseOver()) mainMenuReq = true;
  }
  /**
   ...
   */
  private Character createPlayer(GameContainer gc)
    ...
    Attributes attributes =
        new Attributes(
            strS.getValue(), conS.getValue(), dexS.getValue(), intS.getValue(), wisS.getValue());
    return new Character(
        ...
        attributes,
        portrait,
        gender.getSSName("cloth-1222211-80x90.png"),
        false,
        gc);
  }
}

...
public class GameWorld extends BasicGameState implements SaveElement {
  ...
  public GameWorld(Character player, Chapter chapter) {
    ...
    if (activeScenario == null) throw new NullPointerException("Initial scenario not found");
    else activeScenario.addQuestsToStart(player);
  ...
  public void render(GameContainer container, StateBasedGame game, Graphics g)
    ...
    if (ui != null) {
      ...
      // map
      if (Settings.getMapRenderType().equals("full")) area.getMap().render(0, 0);
      else if (Settings.getMapRenderType().equals("light")) renderLightMap(area.getMap(), g);
      // interface pointers
      ui.drawPointners();
      // objects
      ...
      for (Exit exit : area.getExits()) {
        if (player.isNearby(exit.getPos().toArray())) exit.draw();
      }
      for (Character npc : area.getNpcs()) {
        if (player.isNearby(npc)) npc.draw();
      }
      player.draw();
      if (!Settings.getFowType().equals("off")) drawFOW(g);
      // interface
      g.resetTransform();
      day.draw();
      ui.draw(g);
  ...
  public void update(GameContainer container, StateBasedGame game, int delta)
    ...
    if (ui != null) {
      ...
      if (ui.isExitReq()) container.exit();
    }
    ...
    if (!isPause()) {
      ...
      if (out != CharacterOut.SUCCESS) Log.addWarning(out.toString());
      ...
      if (changeAreaReq && exitToNewArea != null) {
        if (waitForRender <= 0) // to let UI to display appropriate message
        {
          ...
          else changeScenario(exitToNewArea, container, game);
      ...
      if (cui != null) activeScenario.runScripts(cui, delta);
      ...
      if (ui != null) ui.setAlert(TConnector.getText("ui", "pauseAlert"));
    }
  }
  /**
  ...
  public boolean isPause() {
    if (ui != null) return ui.isPauseReq();
    else return false;
  }
  /**
  ...
  private void drawFOW(Graphics g) {
    ...
    for (int i = 0; i < area.getMap().getHeight(); i++) {
      for (int j = 0; j < area.getMap().getWidth(); j++) {
        ...
        if (!player.isNearby(new int[] {x, y})
            && (Settings.getFowType().equals("full")
                || tilePos.isIn(Global.getCameraStartPos(), Global.getCameraEndPos())))
   ...
   * @throws SlickException
   */
  private void changeScenario(Exit exit, GameContainer gc, StateBasedGame game)
      throws SlickException {
    try {
  ...
  private void changeArea(Exit exit) {
    Area area = mainArea;
    if (!exit.isToSub()) area = activeScenario.getMainArea();
    else area = activeScenario.getArea(exit.getSubAreaId());

    if (player.getCurrentArea() != null) player.getCurrentArea().getCharacters().remove(player);
    player.setArea(area);
  ...
  private void printScreen(Graphics g) throws SlickException, IOException {
    Image screenshot =
        new Image((int) Settings.getResolution()[0], (int) Settings.getResolution()[1]);
    g.copyArea(screenshot, 0, 0);

    ImageOut.write(
        screenshot,
        Settings.SCREENSHOTS_DIR + File.separator + "screenshot-" + new Date().toString() + ".jpg");

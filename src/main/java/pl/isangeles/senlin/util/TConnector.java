...
public final class TConnector {
  ...
  /**
   * Static method which searches specified text file in current lang directory and return string
   * with specific id construction of text file: [text id]:[text];[new line mark]
   *
   ...
   * (data/modules/[currentModuleName]/lang/[currentLangName]/[fileName]) construction of text file:
   * [text id]:[text];[new line mark]
   *
   * @param fileName Name of the file in current lang directory
   * @param textID Id of text line
   * @return String with text or error message
  ...
  /**
   * Returns text with for specified ID from specified file in lang directory of active chapter in
   * current module directory (data/modules/[current module name]/chapers/[current chapter
   * name]/lang/[current lang name]/[file name]) construction of text file: [text id]:[text];[new
   * line mark]
   *
   * @param fileName Name of the file in current lang directory
   * @param textID Id of text line
  ...
  /**
   * Returns table with name[0] and info[1] for specified ID form specified lang file in data/lang
   * directory
   *
  ...
  /**
   * Returns table with name[0] and info[1] for specified ID from lang file in current module
   * directory
   *
   * @param langFile Some file in current lang directory
   * @param id ID of line in specified file
  ...
  public static String[] getInfoFromModule(String langFile, String id) {
    ...
    try {
      nameAndInfo[0] =
          getTextFromFile(Module.getGlobalLangPath() + File.separator + langFile, id).split(";")[0];
      nameAndInfo[1] =
          getTextFromFile(Module.getGlobalLangPath() + File.separator + langFile, id).split(";")[1];
    } catch (ArrayIndexOutOfBoundsException e) {
      Log.addSystem("tc_get_info_fail_msg///No name or info in " + langFile + " for: " + id);
    }
    return nameAndInfo;
  ...
  /**
   * Returns table with name[0] and info[1] for specified ID from lang file in active chapter
  ...
  public static String[] getInfoFromChapter(String langFile, String id) {
    ...
    try {
      nameAndInfo[0] =
          getTextFromFile(Module.getLangPath() + File.separator + langFile, id).split(";")[0];
      nameAndInfo[1] =
          getTextFromFile(Module.getLangPath() + File.separator + langFile, id).split(";")[1];
    } catch (ArrayIndexOutOfBoundsException e) {
      Log.addSystem("tc_get_info_fail_msg///No name or info in " + langFile + " for: " + id);
    }
    return nameAndInfo;
  ...
  public static String getRanomText(String category) {
    ...
    try {
      ...
      if (textTab.length < 2) return textTab[0];
      else return textTab[roll.nextInt(textTab.length - 1)];
    } catch (FileNotFoundException e) {

...
public final class RecipeParser {
  ...
  public static Recipe getRecipeFromNode(Node recipeNode) throws NullPointerException {
    ...
    ItemsRequirement reqItems =
        new ItemsRequirement(getComponents(recipeE.getElementsByTagName("components").item(0)));
    List<String> result = getResult(recipeE.getElementsByTagName("result").item(0));
    List<Requirement> requirements =
        RequirementsParser.getReqFromNode(recipeE.getElementsByTagName("trainReq").item(0));

    return new Recipe(id, type, level, reqItems, result, requirements);
  }
  /**
   * Parses specified components node to map with items IDs as keys and amount of these items as
   * values
   *
   ...
   * @throws NullPointerException
   */
  private static Map<String, Integer> getComponents(Node componentsNode)
      throws NullPointerException {

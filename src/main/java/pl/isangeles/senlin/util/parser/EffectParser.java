...
public final class EffectParser {
  ...
  public static EffectPattern getEffectFromNode(Node effectNode) throws NumberFormatException {
    ...
    int duration = 0;
    if (effectE.getAttribute("duration").equals("always")) duration = -1;
    else duration = Integer.parseInt(effectE.getAttribute("duration"));
    ...
    int dot = 0;
    if (dotNode != null) dot = Integer.parseInt(dotNode.getTextContent());

...
public final class NpcParser {
  ...
  public static NpcPattern getNpcFromNode(Node npcNode)
    ...
    // int gold = InventoryParser.getGoldFromNode(inNode); //now gold is represented by in-game
    // items
  ...
  private static List<String> getSkills(Node skillsNode) {
    ...
    if (skillsNode == null) return skills;
  ...
  /**
   * Parses specified effects node to map with effects IDs as keys and current duration time as
   * values UNUSED adding effects to character on NPCs base level is not supported anymore
   *
  ...
  private static Map<String, Integer> getEffects(Node effectsNode) {
    ...
    if (effectsNode == null) return effects;
  ...
  private static List<Profession> getProfessions(Node craftingNode) {
    ...
    if (craftingNode == null) return professions;
  ...
  private static List<Training> getTrainings(Node trainingNode) {
    ...
    if (trainingNode == null) return trainings;
    ...
    if (professionsE != null) {
      ...
      for (int i = 0; i < professionsList.getLength(); i++) {
        ...
        if (professionNode.getNodeType() == javax.xml.soap.Node.ELEMENT_NODE) {
          ...
          List<Requirement> proReqs =
              RequirementsParser.getReqFromNode(
                  professionE.getElementsByTagName("trainReq").item(0));
  ...
  private static List<Dialogue> getDialogues(Node dialoguesNode) {
    ...
    if (dialoguesNode == null) return dialogues;

...
public final class InventoryParser {
   ...
   * @return List with random items patterns (NOTE that if specified node is null then returns empty
   *     list)
   */
  public static List<RandomItem> getItemsFromNode(Node itemsNode) {
    List<RandomItem> items = new ArrayList<>();
    if (itemsNode == null) return items;
    ...
    for (int j = 0; j < itemsNl.getLength(); j++) {
      try {
        ...
        if (itemE.hasAttribute("min")) min = Integer.parseInt(itemE.getAttribute("min"));
        ...
        if (!itemE.hasAttribute("serial")) {
          for (int i = 0; i < amount; i++) {
            RandomItem ip;
            if (i < min) ip = new RandomItem(itemInId, false);
            else ip = new RandomItem(itemInId, random);
  ...
  /**
   * Parses specified inventory node to gold value DEPRECATED gold id now represented by in-game
   * items
   *
   * @param inNode XML inventory node

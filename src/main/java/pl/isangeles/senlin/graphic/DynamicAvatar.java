...
public class DynamicAvatar extends CharacterAvatar {
   ...
   */
  public DynamicAvatar(Character character, GameContainer gc, String spritesheet)
    ...
    if (isStatic()) {
      ...
      defTorso =
          new AnimObject(
              GConnector.getInput("sprite/avatar/" + spritesheet), spritesheet, false, 80, 90);
      defTorso.setName(spritesheet);
      defHead =
          new AnimObject(
              GConnector.getInput(
                  "sprite/avatar/"
                      + character.getGender().getSSName("headBlack-1222211-80x90.png")),
              "headBlackSS" + character.getId(),
              false,
              80,
              90);
    }
  ...
  public void draw(float x, float y) {
    ...
    if (weapon != null) weapon.draw(x, y);
  }
  /**
  ...
  public void kneel() {
    torso.kneel(true);
    head.kneel(true);
    if (weapon != null) weapon.kneel(true);
    defTorso.kneel(true);
    defHead.kneel(true);
  }
  ...
  public void lie() {
    torso.lie(true);
    head.lie(true);
    if (weapon != null) weapon.lie(true);
    defTorso.lie(true);
    defHead.lie(true);
  }
  ...
  public void goRight() {
    torso.goRight();
    head.goRight();
    if (weapon != null) weapon.goRight();
  }

  public void goDown() {
    torso.goDown();
    head.goDown();
    if (weapon != null) weapon.goDown();
    defTorso.goDown();
    defHead.goDown();
  }
  ...
  public void move(boolean move) {
    ...
    head.move(move);
    torso.move(move);
    if (weapon != null) weapon.move(move);
    defTorso.move(move);
    defHead.move(move);
  }
  ...
  public void meleeAnim(boolean loop) {
    ...
    head.meleeAnim(loop);
    if (weapon != null) weapon.meleeAnim(loop);
  ...
  public void castAnim(boolean loop) {
    torso.castAnim(loop);
    head.castAnim(loop);
    if (weapon != null) weapon.castAnim(loop);
  ...
  public void stopAnim() {
    torso.stopAnim();
    head.stopAnim();
    if (weapon != null) weapon.stopAnim();
  }
  /** Resets avatar stance */
  public void resetStance() {
    torso.resetStance();
    head.resetStance();
    if (weapon != null) weapon.resetStance();
  }
  /** Resets avatar */
  public void reset() {
    stopAnim();

...
public class AnimObject extends GameObject {
   ...
   */
  public AnimObject(InputStream is, String ref, boolean flipped, int fWidth, int fHeight)
      throws SlickException {
   ...
   * Draws object
   *
   * @param scale Defines size of object(remember that object is always implicitly scaled to current
   *     resolution)
   */
  @Override
  public void draw(float x, float y) {
    ...
    if (isMove && !meleeReq && !rangeReq && !castReq) {
      move.draw(
          x,
          y,
          (move.getCurrentFrame().getWidth() * getScale()) * sc,
          (move.getCurrentFrame().getHeight() * getScale()) * sc);
    ...
    if (meleeReq) {
      melee.draw(
          x,
          y,
          (melee.getCurrentFrame().getWidth() * getScale()) * sc,
          (melee.getCurrentFrame().getHeight() * getScale()) * sc);
      if (melee.isStopped()) meleeReq = false;
    }
    if (rangeReq) {
      range.draw(
          x,
          y,
          (range.getCurrentFrame().getWidth() * getScale()) * sc,
          (range.getCurrentFrame().getHeight() * getScale()) * sc);
      if (range.isStopped()) rangeReq = false;
    }
    if (castReq) {
      cast.draw(
          x,
          y,
          (cast.getCurrentFrame().getWidth() * getScale()) * sc,
          (cast.getCurrentFrame().getHeight() * getScale()) * sc);
      if (cast.isStopped()) castReq = false;
    }
  }
  /**
  ...
  public void update(int delta) {
    ...
    range.update(delta);
    cast.update(delta);
    if (meleeReq || rangeReq) animTime += delta;
    if (animTime > 300) {
      animProgress += 0.3f;
      animTime = 0f;
    }
  ...
  public Sprite getCurrentSprite() {
    if (isMove) return (Sprite) move.getCurrentFrame();
    else return idle;
  }
  /**
  ...
  public int getDirection() {
    if (move == moveUp || idle == idleUp) return Coords.UP;
    if (move == moveRight || idle == idleRight) return Coords.RIGHT;
    if (move == moveDown || idle == idleDown) return Coords.DOWN;
    if (move == moveLeft || idle == idleLeft) return Coords.LEFT;

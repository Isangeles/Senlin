...
public abstract class CharacterAvatar implements MouseListener, Effective {
   ...
   */
  public CharacterAvatar(Character character, GameContainer gc)
  ...
  public void draw(float x, float y) {
    ...
    if (isSpeaking && speechTime < 1500) speakWindow.draw(x, y);
  ...
  public void update(int delta) {
    ...
    for (SimpleAnim effect : effects) {
      if (effect.isLastFrame()) effectsToRemove.add(effect);
    }
  ...
  public boolean addEffect(SimpleAnim effect, boolean loop) {
    if (effect != null) {
      if (loop) return loopEffects.add(effect);
      else return effects.add(effect);
    } else return false;
  }
  /* (non-Javadoc)

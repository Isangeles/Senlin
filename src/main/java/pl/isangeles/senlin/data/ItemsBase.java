...
public class ItemsBase {
  ...
  public static Item getItem(String id) {
    ...
    try {
      if (weaponsMap.get(id) != null) return weaponsMap.get(id).make(gc);

      if (armorsMap.get(id) != null) return armorsMap.get(id).make(gc);

      if (trinketsMap.get(id) != null) return trinketsMap.get(id).make(gc);

      if (miscMap.get(id) != null) return miscMap.get(id).make(gc);
  ...
  public static Item getItem(String id, long serial) {
    ...
    try {
      if (weaponsMap.get(id) != null) return weaponsMap.get(id).make(gc, serial);

      if (armorsMap.get(id) != null) return armorsMap.get(id).make(gc, serial);

      if (trinketsMap.get(id) != null) return trinketsMap.get(id).make(gc, serial);

      if (miscMap.get(id) != null) return miscMap.get(id).make(gc, serial);
   ...
   */
  public static void load(String itemsPath, GameContainer gc)
      throws SlickException, IOException, FontFormatException, ParserConfigurationException,
          SAXException {
    ItemsBase.gc = gc;

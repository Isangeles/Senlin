...
public class ObjectPattern {
   ...
   */
  public TargetableObject make(GameContainer gc)
      throws SlickException, IOException, FontFormatException {
    Portrait uiPortrait = new Portrait(GConnector.getObjectPortrait(portrait), gc);
    Sound objectSound = null;
    if (sound != "") objectSound = new Sound(AConnector.getInput("effects/" + sound), sound);

    TargetableObject object = null;
    if (anim) {
      SimpleAnim animTexture =
          new SimpleAnim(
              GConnector.getInput("object/anim/" + mainTexture),
              id,
              flipped,
              fWidth,
              fHeight,
      ...
      else object = new TargetableObject(id, animTexture, uiPortrait, actionP.make());
    } else {
      Sprite staticTexture =
          new Sprite(GConnector.getInput("object/static/" + mainTexture), id, flipped, info, gc);
      if (objectSound != null)
        object = new TargetableObject(id, staticTexture, uiPortrait, objectSound, actionP.make());
      else object = new TargetableObject(id, staticTexture, uiPortrait, actionP.make());
    }
